...
class Dungeon:
    def __init__(self, w, h, debug=False):
        ...
        self.rooms = np.zeros((h, w), dtype=bool)
        self.rooms_named = np.zeros((h, w), dtype=bool)
        
        self.corridors = np.zeros((h, w), dtype=bool)        
        self.entrances = []
    ...
    def create_map_dungeon(self, num_col_rooms=3, 
        ...
        rooms = np.ones(self.data.shape, np.int) # 壁が1、通路が0
        rooms_named = np.zeros(self.data.shape, np.int) # 壁が0, 部屋が1以上の数字
        corridors = np.ones(self.data.shape, np.int) # 通路が0
        ...
        for ic in range(num_col_rooms):
            for ir in range(num_row_rooms):
                ...
                rooms_named[cy-int(h/2.0):cy+int(h/2.0), cx-int(w/2.0):cx+int(w/2.0)] = room_idx
        ...
        if corridor_width > 1:            
            ...
            dilated_corriders = dilation(corridors, ksize=corridor_width)            
            corridors = np.logical_xor(dilated_corriders.astype(np.bool), ~(corridors).astype(np.bool))

        # combine rooms and corridors
        self.data = np.logical_and(rooms, corridors).astype(np.int)
        self.rooms = rooms.astype(np.bool)
        self.rooms_named = rooms_named
        self.corridors = (corridors.astype(np.bool) + ~rooms.astype(np.bool))
    ...
    def get_pos_from_corridors(self, num=1):
        """
        if num == 1:
            return (y, x)
        else:
            return [(y1, x1), (y2, x2), ...]
        """
        idx = list(zip(*np.where(self.corridors==0)))
        yx = random.sample(idx, num)
    ...
    def get_pos_from_rooms(self, num=1):
        """
        if num == 1:
            return (y, x)
        else:
            return [(y1, x1), (y2, x2), ...]
        """
        idx = list(zip(*np.where(self.rooms==0)))
        yx = random.sample(idx, num)
...
if __name__ == "__main__":
    ...
    dungeon.create_map_dungeon(num_col_rooms=2, num_row_rooms=2, corridor_width=3)
